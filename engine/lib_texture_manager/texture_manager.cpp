...
const sf::Texture& TextureManager::getTexture(const std::string& filename)
{
	...
	for(std::vector<std::string>::const_iterator it = _resource_directories.begin();
		it != _resource_directories.end();
		++it)
	{
		if(tex.loadFromFile((*it) + filename))
		{
			_textures[filename] = tex;
			LOG(INFO) << "TextureManager: texture " << filename
					  << " found in registered resource directory " << *it;
			return _textures[filename];
		}
	}
