...
void AudioManager::playSound(const std::string& name)
{
	if(_sound)
	{
		for(std::map<std::string, std::shared_ptr<Sound>>::const_iterator it = _sounds.begin();
			it != _sounds.end();
			++it)
		{
			if(name == it->first)
...
{
	for(std::map<std::string, std::shared_ptr<SoundBuffer>>::const_iterator it = _buffers.begin();
		it != _buffers.end();
		++it)
	{
		if(it->first == name)
...
void AudioManager::toggleSoundOnOff() { _sound = !_sound; }

bool AudioManager::isSoundOn() { return _sound; }
