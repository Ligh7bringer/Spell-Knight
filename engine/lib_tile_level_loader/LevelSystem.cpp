...
LevelSystem::Tile LevelSystem::getGroundTile(sf::Vector2ul p)
{
	...
	{
		if(_tiles[i] == '1' || _tiles[i] == '2' || _tiles[i] == '3' || _tiles[i] == '4' ||
		   _tiles[i] == '5' || _tiles[i] == '6' || _tiles[i] == '7' || _tiles[i] == '8' ||
		   _tiles[i] == '9' || _tiles[i] == '0' || _tiles[i] == 'I' || _tiles[i] == 'O' ||
		   _tiles[i] == 'P')
		{
			return _tiles[(p.y * _width) + p.x];
		}
...
{
	LOG(INFO) << "LevelSystem unloading\n";
	_sprites.clear();
	_texs.clear();
	_tiles.reset();
	_width = 0;
...
//use this instead of finding tiles first and then getting their positions
std::vector<sf::Vector2f> LevelSystem::getPosition(LevelSystem::Tile t)
{
	vector<Vector2f> positions;

	auto tiles = findTiles(t);
	for(auto tile : tiles)
	{
		auto tilePos = getTilePosition(tile);
		positions.push_back(tilePos);
	}
