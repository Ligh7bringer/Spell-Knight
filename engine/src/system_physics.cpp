...
void update(const double& dt) { world->Step((float)dt, velocityIterations, positionIterations); }
...
const Vector2f invert_height(const Vector2f& in) { return Vector2f(in.x, 720 - in.y); }

float getPhysicsScale() { return physics_scale; }

float getPhysicsScaleInverted() { return physics_scale_inv; }
