...
void SettingsParser::saveToFile(bool overwrite)
{
	...
	else
	{
		...
		for(auto& it : _data) fileContents.emplace_back(it.first, it.second);
	}
	//close stream
	in.close();
...
SettingsParser::~SettingsParser() { saveToFile(true); }
...
void SettingsParser::put(const std::string& key, const std::string& value) { _data[key] = value; }
