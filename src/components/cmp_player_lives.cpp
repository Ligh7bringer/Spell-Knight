...
PlayerLivesComponent::PlayerLivesComponent(Entity* p, unsigned int lives)
	: Component(p)
	, _maxLives(5)
	, _lives(lives)
...
void PlayerLivesComponent::render() { _panel.render(); }
...
//returns lives
int PlayerLivesComponent::getLives() const { return _lives; }
