	...
	if(auto player = _player.lock())
	{
		...
		if(sf::length(player->getPosition() - _parent->getPosition()) < 25.0f)
		{
			...
			if(_givesHealth)
			{
				...
				auto particles = player->addComponent<ParticleSystemComponent>(
					ParticleSystem::FLOATING, "healing-particle.png");
			...
			{
				auto particles = player->addComponent<ParticleSystemComponent>(
					ParticleSystem::FLOATING, "money-particle.png");
				particles->setRepeat(false);
...
PickUpComponent::PickUpComponent(Entity* p, unsigned int points)
	: Component(p)
