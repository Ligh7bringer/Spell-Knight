...
AnimatedSpriteComponent::AnimatedSpriteComponent(Entity* p, Vector2f size)
	: Component(p)
	, _size(size)
	, _rotation(0)
{
...
void AnimatedSpriteComponent::render() { Renderer::queueAndOptimise(_sprite); }
...
void AnimatedSpriteComponent::setFrameTime(float t) { _frameTime = t; }

//sets number of frames the animation consists of
void AnimatedSpriteComponent::setNumberOfFrames(int num) { _numOfFrames = num; }
...
//sets padding
void AnimatedSpriteComponent::setSpriteSheetPadding(int padding) { _spriteSheetPadding = padding; }

//sets whether the entity is facing right or not
void AnimatedSpriteComponent::setFacingRight(bool b) { _facingRight = b; }
...
const Vector2f& AnimatedSpriteComponent::getSize() const { return _size; }

//get or set if the entity is hurt
void AnimatedSpriteComponent::setHurt(bool h) { _hurt = h; }

//returns whether a hurt animation is being played
bool AnimatedSpriteComponent::isHurt() const { return _hurt; }

//returns whether the entity is facing right
bool AnimatedSpriteComponent::isFacingRight() const { return _facingRight; }

//returns the number of frames in current animation
int AnimatedSpriteComponent::getFrameCount() const { return _numOfFrames; }

//returns the speed of the current animation
float AnimatedSpriteComponent::getFrameTime() const { return _frameTime; }

bool AnimatedSpriteComponent::isDone() const { return _done; }

const Sprite& AnimatedSpriteComponent::getSprite() const { return _sprite; }

void AnimatedSpriteComponent::rotate(float deg) { _sprite.rotate(deg); }

void AnimatedSpriteComponent::setRotation(float angle) { _rotation = angle; }
