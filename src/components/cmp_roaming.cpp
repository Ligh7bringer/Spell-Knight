...
RoamingComponent::RoamingComponent(Entity* p, const Vector2f& dist, float time, bool platform) : Component(p),
                                                ...
                                                _physics_scale(Physics::getPhysicsScale()), _isPlatform(platform) {}
    ...
    if(_isPlatform) {
        if (auto player = _player.lock()) {
            ...
            if (physics->isTouching(*playerPhysics)) {
                //calculate velocity
                auto targetVel = playerPhysics->getMass() * (physics->getVelocity() - playerPhysics->getVelocity()) /
                                 _physics_scale;
                //ignore y velocity
                targetVel.y = 0;
                //impulse
...
void RoamingComponent::setTravelDistance(const Vector2f& dist) {
    _distance = dist;
}
