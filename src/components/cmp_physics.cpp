	...
	, _dynamic(dyn)
{
	...
	_body->SetActive(true);
	{
		...
		// Fixture properties
		FixtureDef.density = _dynamic ? 0.5f : 0.f;
		FixtureDef.friction = _dynamic ? 0.1f : 0.5f;
...
PhysicsComponent::PhysicsComponent(Entity* p, const sf::Vector2f& size)
	: Component(p)
	, _size(size)
	, _dynamic(false)
{
...
PhysicsComponent::PhysicsComponent(Entity* p)
	: Component(p)
	, _dynamic(false)
{
	//create body def
...
void PhysicsComponent::setRestitution(float r) { _fixture->SetRestitution(r); }
...
void PhysicsComponent::setGravityScale(int32 gs) { _body->SetGravityScale(gs); }

b2ContactEdge* PhysicsComponent::getContactList() const { return _body->GetContactList(); }

const Vector2f& PhysicsComponent::getSize() const { return _size; }

const b2Vec2& PhysicsComponent::getPosition() const { return _body->GetPosition(); }

float32 PhysicsComponent::getMass() const { return _body->GetMass(); }
