...
EntityFactory::makeSlime(Scene* scene, const Vector2f& pos, const Vector2f& distance, float time)
{
...
std::shared_ptr<Entity> EntityFactory::makeEyeDemon(Scene* scene, const sf::Vector2f& pos)
{
	...
	sm->addState("sleeping",
				 std::make_shared<SleepingState>(scene->ents.find("player")[0], "flying"));
...
std::shared_ptr<Entity> EntityFactory::makeMovingPlatform(Scene* scene,
