...
Menu::Menu() {
    settings.readFile("res/settings.cfg");
    language.readFile(settings.get("language"));

    //Play menu item 
    MenuItem playButton;
    playButton.button = Button(Vector2f(500, 350), Vector2f(200, 35), language.get("play"));
    playButton.action = PLAY;

    //options menu item
    MenuItem optionsButton;
    optionsButton.button = Button(Vector2f(500, 400), Vector2f(200, 35), language.get("options"));
    optionsButton.action = OPTIONS;

    //exit menu item 
    MenuItem exitButton;
    exitButton.button = Button(Vector2f(500, 450), Vector2f(200, 35), language.get("exit"));
