...
void GameOverScene::Load()
{
	...
	restartLevelbtn = Button(Vector2f(pos.x - 100, pos.y + 100),
							 Vector2f(200.f, 35.f),
							 Config::getLocalisedString("restart"));
	mainMenubtn = Button(Vector2f(pos.x - 300, pos.y + 100),
						 Vector2f(200.f, 35.f),
						 Config::getLocalisedString("main_menu"));
...
void GameOverScene::setText(const std::string& text) { _message = text; }
...
void GameOverScene::nextLevel()
{
	...
	Vector2f pos(Engine::getWindowSize());
	pos /= 2.f;
	nextLevelbtn = Button(Vector2f(pos.x + 100, pos.y + 100),
						  Vector2f(200.f, 35.f),
						  Config::getLocalisedString("next_level"));
}
